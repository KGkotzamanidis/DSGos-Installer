...
""" This script generates an update.info file used to update DSGos """
...
def create_update_info():
    ...
    myfiles = get_files("/usr/share/DSGos") or get_files(".")

    txt = '{"version":"%s","files":[\n' % info.DSGos_VERSION

    for filename in myfiles:
        md5 = get_md5(filename)
        if "usr/share/DSGos" not in filename:
            filename = filename.replace('./', '/usr/share/DSGos/')
        txt += '{"name":"%s","md5":"%s"},\n' % (filename, md5)
