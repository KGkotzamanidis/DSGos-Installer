...
set_xorg() {
	cp /usr/share/DSGos/scripts/postinstall/50-synaptics.conf ${CN_DESTDIR}/etc/X11/xorg.conf.d/50-synaptics.conf
	cp /usr/share/DSGos/scripts/postinstall/99-killX.conf ${CN_DESTDIR}/etc/X11/xorg.conf.d/99-killX.conf

	# Fix sensitivity for chromebooks
	if lsmod | grep -q cyapa; then
		cp /usr/share/DSGos/scripts/postinstall/50-cros-touchpad.conf ${CN_DESTDIR}/etc/X11/xorg.conf.d/50-cros-touchpad.conf
	fi
}

set_gsettings() {
	cp /usr/share/DSGos/scripts/set-settings "${CN_DESTDIR}/usr/bin/set-settings"
...
gnome_settings() {
	...
	if [[ "${CN_KEYBOARD_VARIANT}" != '' ]]; then
		sed -i "s/'us'/'${CN_KEYBOARD_LAYOUT}+${CN_KEYBOARD_VARIANT}'/" /usr/share/DSGos/scripts/set-settings
	else
		sed -i "s/'us'/'${CN_KEYBOARD_LAYOUT}'/" /usr/share/DSGos/scripts/set-settings
	fi
...
## Set default directories
	...
	# xscreensaver config
	cp /usr/share/DSGos/scripts/postinstall/xscreensaver ${CN_DESTDIR}/home/${CN_USER_NAME}/.xscreensaver
...
cinnamon_settings() {
	...
	cp -f /usr/share/DSGos/scripts/postinstall/menu@cinnamon.org.json ${CN_DESTDIR}/home/${CN_USER_NAME}/.cinnamon/configs/menu@cinnamon.org/

	# Copy panel-launchers@cinnamon.org.json to set launchers
	if [[ firefox = "${CN_BROWSER}" ]]; then
		sed -i 's|chromium|firefox|g' /usr/share/DSGos/scripts/postinstall/panel-launchers@cinnamon.org.json
	fi
	mkdir -p ${CN_DESTDIR}/home/${CN_USER_NAME}/.cinnamon/configs/panel-launchers@cinnamon.org/
	cp -f /usr/share/DSGos/scripts/postinstall/panel-launchers@cinnamon.org.json ${CN_DESTDIR}/home/${CN_USER_NAME}/.cinnamon/configs/panel-launchers@cinnamon.org/
...
# Set gsettings
	...
	if [[ $CN_BROWSER = "firefox" ]]; then
		sed -i 's|chromium|firefox|g' /usr/share/DSGos/scripts/postinstall/applications.list
	fi
	cp /usr/share/DSGos/scripts/postinstall/applications.list ${CN_DESTDIR}/usr/lib/linuxmint/mintMenu/applications.list
...
postinstall() {
	...
	if [ -f "${FONTCONFIG_FILE}" ]; then
		FONTCONFIG_FILE="/usr/share/DSGos/scripts/fonts.conf"
